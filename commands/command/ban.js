...
module.exports = {
    ...
    async run (client, message, args) {
            ...
            message.guild.member(user).ban().catch(err=>{
              if(err){
                message.channel.send({content:"I cant ban the user because he is the owner of the server!"}).then(msg=>msg.delete({timeout:10000})); 
              }
            })
            message.channel.send({content:`Successfully Banned ${user}. Reason: ${reason}`})
